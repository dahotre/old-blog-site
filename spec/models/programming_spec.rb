...
describe Programming do
  ...
  it "Should create a programming instance given valid params" do
    p = Programming.new(@params)
    p.save
    p1 = Programming.where("url_title = ?", "url-title-test").last
    p1.should_not be_nil
    p1.should be_valid
    p.should == p1
  end

  it "Should access previously stored records in db" do
    ps = Programming.where("author = ?", "Test AUth")
    ps.should_not be_nil
    ps.size.should == 2
  end

  it "Should return url_title of previous prog node" do
    ...
    currProg = NodeFactory.instance.get_programming_node(url_titles[1])
    currProg.get_previous_url_title.should_not be_nil
    currProg.get_previous_url_title.should_not == url_titles[0]
  end

  it "Should return url_title of next prog node" do
    params = {:title => "rspec is awesome", :html => "<p>Hello world from awesome rspec</p>", :author => "rspec auth"}
    url_titles = ["url-title-rspec-1", "url-title-rspec-2", "url-title-rspec-3"]
    url_titles.each{|url_title| Programming.create(params.merge(:url_title => url_title))}
    currProg = NodeFactory.instance.get_programming_node(url_titles[1])
    currProg.get_next_url_title.should_not be_nil
    currProg.get_next_url_title.should_not == url_titles[2]
  end
  
  it "Should create a programming instance & tags given valid params" do
    p = Programming.new(@params)
